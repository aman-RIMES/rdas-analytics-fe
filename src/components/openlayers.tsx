...
const OpenLayersMap = ({
  ...
  useEffect(() => {
    ...
    if (geoJsonData && geoJsonData.features) {
      try {
        ...
        const styleFunction = (feature: any) => {
          const districtCode = feature.get(geoJsonStructure[country]?.district_code);
          let value;
          
          try {
            value = mapType === mapDataType.normal
              ? feature.get(mapFilter?.dataVariable)?.normal[mapFilter?.chosenMonth - 1]
              : feature.get(mapFilter?.dataVariable)?.anomaly[mapFilter?.chosenMonth - 1][chosenYear];
          } catch (error) {
    ...
    const map = new Map({
      ...
      view: new View({
        center: subjectCountry?.coordinates ? fromLonLat([subjectCountry.coordinates[1], subjectCountry.coordinates[0]]) : fromLonLat([0, 0]),
    ...
    if (vectorSource && vectorSource.getFeatures().length > 0) {
      map.on('pointermove', (evt) => {
        ...
        if (feature) {
          try {
            const districtName = feature.get(geoJsonStructure[country]?.district_name) || '';
            const provinceName = feature.get(geoJsonStructure[country]?.province_name) || '';
            const value = mapType === mapDataType.normal
              ? feature.get(mapFilter?.dataVariable)?.normal[mapFilter?.chosenMonth - 1]
              : feature.get(mapFilter?.dataVariable)?.anomaly[mapFilter?.chosenMonth - 1][chosenYear];
            ...
            if (popupElement.current) {
              popupElement.current.innerHTML = `
                ...
                ${provinceName || '--'} : ${parseFloat(value)?.toFixed(2) || '--'} mm
              `;
  ...
  }, [geoJsonData, country, mapType, chosenYear, chosenDistrict, preferredZoomScale, mapFilter]);

  return (
    <div className="relative">
      <div 
        ref={mapElement} 
        className="w-full h-[400px] z-1"
      />
      <div 
        ref={popupElement}
      ...
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 w-[97%] bg-white/80 p-1 rounded-lg z-10">
        <MapLegend mapType={mapType || 'normal'} variable={mapFilter?.dataVariable || 'rainfall'} />
        <p className='text-center text-xs'>{`${formatTitle(mapType || '')} ${formatTitle(mapFilter?.dataVariable || 'rainfall')} ${mapFilter?.dataVariable === "rainfall" ? "(mm)" : "(°C)"}`}</p>
      </div>
    </div>
  );
};
